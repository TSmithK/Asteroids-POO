...
import asteroids.entity.satelite.Poder;
 ...
 *
 */
public class Asteroid extends EntityEnemy {
    ...
    private static int probabilidad = 40;

    @Override
    public void handleCollision(Game game, Entity other) {
        //Prevent collisions with other asteroids.
        if (other.getClass() != Asteroid.class && other.getClass() != Poder.class) {
            ...
            // la probabilidad que aparezca un power up va a bajar en cuanto se vaya subiendo de nivel
            int comparador = probabilidad - (game.getLevel() * 3);
            if(comparador < 10) { // si es menor a 10, redondeamos a 10
                comparador = 10;// para que la probabilidad menor maximo siempre sea 10
            } 
            if (comparador > obtenerNumeroRandom(1, 100)) {
                game.registerEntity(new Poder(this, size.radius));                
            }
            flagForRemoval();
    ...
    private static int ciclos = 5;
    public int obtenerNumeroRandom(int min, int max) {
        int sumatoria = 0;
        Random random = new Random();
        for(int i = 0; i < ciclos;i++) {        
        sumatoria = sumatoria + random.nextInt(max - min) + min;
        }
        return sumatoria/ciclos;
    }
}
